...
from .periodic_pdf import generate_periodic_pdf
from .yearly_pdf import generate_yearly_pdf
...
def get_template_path(template_name=None, pass_template=True, conditional=False):
    """
    ...
    if conditional:
        template_path = YEARLY_CONDITIONAL_TEMPLATE
    else:
        template_path = YEARLY_PASS_TEMPLATE if pass_template else YEARLY_FAIL_TEMPLATE
...
def generate_gradesheet_pdf(level_id, student_id=None, academic_year_id=None):
    """
    Generate periodic PDF grade sheets by calling periodic_pdf.py.
    
    Args:
        level_id: ID of the level.
        student_id: ID of the student (optional).
        academic_year_id: ID of the academic year (optional).
        semester: '1' for 1st semester, '2' for 2nd semester, None for both.
    ...
    try:
        ...
        return generate_periodic_pdf(level_id, student_id, academic_year_id)
